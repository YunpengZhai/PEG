...
# model_urls = {
#     'densenet121_ibn_a': './logs/pretrained/densenet121_ibn_a.pth',
#     'densenet169_ibn_a': './logs/pretrained/densenet169_ibn_a.pth',
# }
...
def densenet121_ibn_a(pretrained=False, **kwargs):
    ...
    model = DenseNet_IBN(num_init_features=64, growth_rate=32, block_config=(6, 12, 24, 16),
                     **kwargs)
    if pretrained:
        # state_dict = torch.load(model_urls['densenet121_ibn_a'], map_location=torch.device('cpu'))['state_dict']
...
def densenet169_ibn_a(pretrained=False, **kwargs):
    ...
    model = DenseNet_IBN(num_init_features=64, growth_rate=32, block_config=(6, 12, 32, 32),
                     **kwargs)
    if pretrained:
        # state_dict = torch.load(model_urls['densenet169_ibn_a'], map_location=torch.device('cpu'))['state_dict']
        # state_dict = remove_module_key(state_dict)
        # model.load_state_dict(state_dict)
